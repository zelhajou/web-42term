          ...
          <div className="space-y-6">
                ...
                {error && (
                  <div className="p-3 bg-red-900/20 border border-red-500/30 rounded-lg text-red-400 text-xs">
                    <div className="flex items-start">
                      <Info size={14} className="text-red-400 mr-1.5 mt-0.5 flex-shrink-0" />
                      {error}
                    </div>
                  </div>
                )}
            ...
            <div className="bg-[#161B22] rounded-lg p-4 border border-gray-800 shadow-md">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex items-start">
                  <span className="flex-shrink-0 flex items-center justify-center w-5 h-5 rounded-full bg-[#1E293B] text-indigo-400 text-xs mr-2 mt-0.5">1</span>
                  <span className="text-gray-300">Enter your 42 intra username</span>
                </div>
                <div className="flex items-start">
                  <span className="flex-shrink-0 flex items-center justify-center w-5 h-5 rounded-full bg-[#1E293B] text-pink-400 text-xs mr-2 mt-0.5">2</span>
                  <span className="text-gray-300">Select visualization type (Profile, Projects, Skills)</span>
                </div>
                <div className="flex items-start">
                  <span className="flex-shrink-0 flex items-center justify-center w-5 h-5 rounded-full bg-[#1E293B] text-cyan-400 text-xs mr-2 mt-0.5">3</span>
                  <span className="text-gray-300">Copy the markdown to your GitHub README</span>
                </div>
              </div>
    ...
    <div className="flex-grow flex flex-col justify-center items-center relative overflow-hidden bg-gradient-to-b from-[#0A0C10] to-[#0F1723]">
      ...
      <div className="absolute inset-0 overflow-hidden opacity-5">
        <div className="typing-animation font-mono text-sm text-gray-400 p-8">
          ...
          <div>-rwxr-xr-x  1 user  staff   285 Mar 29 10:42 profile.terminal</div>
          <div>-rwxr-xr-x  1 user  staff   312 Mar 29 10:42 projects.terminal</div>
          <div>-rwxr-xr-x  1 user  staff   248 Mar 29 10:42 skills.terminal</div>
      ...
      <div className="z-10 py-6 w-full max-w-xl">
        <div className="w-full bg-[#161B22] border border-gray-800 rounded-md shadow-lg overflow-hidden mx-auto">
          <div className="bg-[#1E293B] px-3 py-1.5 border-b border-gray-800 flex items-center justify-between">
            <div className="text-xs text-gray-500 font-mono">command</div>
          ...
          <div className="p-4 font-mono text-gray-300 text-sm space-y-2">
            <div className="flex items-start">
              <span className="text-indigo-500 mr-2 w-4 flex-shrink-0">$</span>
              <span>
                pwd
              </span>
            </div>
            ...
            <div className="flex items-start">
              ...
              <span className="typing-terminal">
                ./generate.sh --user=<span className="text-cyan-400">username</span> --type=<span className="text-pink-400">{displayType}</span> --theme=<span className="text-green-400">{selectedTheme}</span>
                <span className={`inline-block w-2 h-5 bg-indigo-400 ml-0.5 ${showCursor ? 'opacity-100' : 'opacity-0'}`}></span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="z-10 mt-8 flex justify-center gap-8 text-gray-600 text-xs">
        <button 
          onClick={() => setDisplayType('student')}
          className={`flex flex-col items-center gap-1 ${displayType === 'student' ? 'text-indigo-400' : 'hover:text-indigo-400'} transition-colors`}
        >
          <User size={20} />
          <span>profile</span>
        </button>
        <button
          onClick={() => setDisplayType('projects')}
          className={`flex flex-col items-center gap-1 ${displayType === 'projects' ? 'text-pink-400' : 'hover:text-pink-400'} transition-colors`}
        >
          <FolderKanban size={20} />
          <span>projects</span>
        </button>
        <button
          onClick={() => setDisplayType('skills')}
          className={`flex flex-col items-center gap-1 ${displayType === 'skills' ? 'text-cyan-400' : 'hover:text-cyan-400'} transition-colors`}
        >
          <CircuitBoard size={20} />
          <span>skills</span>
        </button>
